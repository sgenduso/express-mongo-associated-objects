...
router.get('/', function(req, res, next) {
  ...
  people.find({}).then(function (people) {
    var personAddressIds = people.map(function (person) {
    ...
    employers.find({_id: {$in: employerIds}}).then(function (employers) {
      ...
      addresses.find({_id: {$in: personAddressIds.concat(employerAddressIds)}}).then(function (allAddresses) {
        joinPersonAddress(people, allAddresses)
        joinEmployerAddress(employers, allAddresses)
